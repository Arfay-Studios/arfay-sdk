...
abstract class SortFilterGraphical<T>(
   ...
   override var sorterEngine = item(Materials.CHEST, "§dOrdem").toSorterEngine(midSlot(lines) + 1)
   
   override var filterOptions = IndexList<FilterPair<T>>()
   override var isFilterDisabled = false
   override var filterEngine = item(Materials.HOPPER, "§dFiltro").toFilterEngine(midSlot(lines) - 1)
