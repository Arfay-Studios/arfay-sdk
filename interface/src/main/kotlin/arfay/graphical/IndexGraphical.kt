...
abstract class IndexGraphical<T>(title: String, size: Int = 6) : ScrollGraphical(title, size) {
   ...
   open var emptyEngine = emptyIndexEngine(midSlot(size / 2), item(Materials.BARRIER, "§cVazio."))
   ...
   override fun scrollTo(to: Int) {
      if (isEmpty) {
         ...
         scrollDownEngine.isVisible = false
         scrollUpEngine.isVisible = false
      } else {
         emptyEngine.uninstall()
      }
