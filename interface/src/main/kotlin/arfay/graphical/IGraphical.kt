 ...
 */
interface IGraphical : Interface, Accessible, Renderable, Observable, Tickable {
   ...
   override fun access(player: Player) {
      ...
      if (!permits(ObserverKind.ACCESS))
         return
   ...
   override fun tick() {
      ticks++
      tickers.forEach { it(this) }
      engineStack.forEachValue { it.tick() }
   }
   
   override fun render() {
      renders.forEach { it() }
      engineStack.forEachValue { it.render() }
      rendereds++
   }
   ...
   operator fun get(index: Int) = engineStack[index]
   operator fun set(index: Int, engine: Engine) = install(index, engine)
   
   /**
    * Returns if in [index] has a valid item.
    */
   override operator fun contains(index: Int) = engineStack.containsKey(index)
...
fun IGraphical.tickInterface() {
   for (ticker in tickers) ticker(this)
}
