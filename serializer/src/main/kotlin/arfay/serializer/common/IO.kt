 ...
 */
fun DataOutput.writeSlot(slot: arfay.core.utils.Slot) {
   writeItem(slot.item)
   writeByte(slot.slot)
}
 ...
 */
fun DataInput.readSlot(): arfay.core.utils.Slot = readItem() slot readByte().toInt()
...
fun DataInput.readInventory(): Inventory {
   ...
   val count = readByte()
   for (i in 0 until count) readSlot().add(inv)
   return inv
}
