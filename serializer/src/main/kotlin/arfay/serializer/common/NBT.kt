...
fun NBTTagList.fromMinecraft(): NbtList<NbtTag> = buildNbtList {
   ...
   val list = listField.get(this@fromMinecraft) as List<NBTBase>
   for (value in list) {
      method.invoke(this, value.fromMinecraft())
   }
}

internal val listField: Field by lazy {
   NBTTagList::class.java.getDeclaredField("list").apply {
      isAccessible = true
   }
}

internal val addListMethod: Method by lazy {
   NbtListBuilder::class.java.getDeclaredMethod("add").apply {
      isAccessible = true
   }
}
 ...
 */
fun <T : NbtTag> NbtList<T>.toMinecraft(): NBTTagList {
   val list = NBTTagList()
   for (value in this) {
      list.add(value.toMinecraft())
   }
   return list
}
