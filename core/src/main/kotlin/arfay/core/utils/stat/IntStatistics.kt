...
fun <T, C : Comparable<C>> BinModel<List<T>, C>.averageByInt(selector: (T) -> Int): BinModel<Double, C> = BinModel(bins.map {
   Bin(
...
fun <T, C : Comparable<C>> BinModel<List<T>, C>.geometricMeanByInt(selector: (T) -> Int): BinModel<Double, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).geometricMean()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.medianByInt(selector: (T) -> Int): BinModel<Double, C> = BinModel(bins.map {
   Bin(
...
fun <T, C : Comparable<C>> BinModel<List<T>, C>.varianceByInt(selector: (T) -> Int): BinModel<Double, C> = BinModel(bins.map {
   Bin(
...
fun <T, C : Comparable<C>> BinModel<List<T>, C>.normalizeByInt(selector: (T) -> Int): BinModel<DoubleArray, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).normalize()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.descriptiveStatisticsByInt(selector: (T) -> Int): BinModel<Descriptives, C> = BinModel(
...
fun <K> Map<K, List<Int>>.descriptiveStatistics(): Map<K, Descriptives> = entries.associate { it.key to it.value.descriptiveStatistics }
