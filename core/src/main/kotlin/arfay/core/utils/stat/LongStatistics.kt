...
inline fun <T, G> Sequence<T>.binByLong(
   binSize: Long, crossinline valueSelector: (T) -> Long, crossinline groupOp: (List<T>) -> G, rangeStart: Long? = null
...
fun <T, C : Comparable<C>> BinModel<List<T>, C>.sumByLong(selector: (T) -> Long): BinModel<Long, C> = BinModel(bins.map {
   Bin(
...
fun <T, C : Comparable<C>> BinModel<List<T>, C>.geometricMeanByLong(selector: (T) -> Long): BinModel<Double, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).geometricMean()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.medianByLong(selector: (T) -> Long): BinModel<Double, C> = BinModel(bins.map {
   Bin(
...
fun <T, C : Comparable<C>> BinModel<List<T>, C>.sumOfSquaresByLong(selector: (T) -> Long): BinModel<Double, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).sumOfSquares()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.normalizeByLong(selector: (T) -> Long): BinModel<DoubleArray, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).normalize()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.descriptiveStatisticsByLong(selector: (T) -> Long): BinModel<Descriptives, C> = BinModel(
...
fun <K> Map<K, List<Long>>.descriptiveStatistics(): Map<K, Descriptives> = entries.associate { it.key to it.value.descriptiveStatistics }
...
fun <K, V> Map<K, List<V>>.descriptiveStatisticsByLong(selector: (V) -> Long): Map<K, Descriptives> = entries.associate {
