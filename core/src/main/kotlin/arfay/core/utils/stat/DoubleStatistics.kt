package arfay.core.utils.stat

import arfay.core.utils.stat.range.*
...
fun <T, C : Comparable<C>> BinModel<List<T>, C>.averageByDouble(selector: (T) -> Double): BinModel<Double, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).average()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.doubleRangeBy(selector: (T) -> Double): BinModel<ClosedFloatingPointRange<Double>, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).doubleRange()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.geometricMeanByDouble(selector: (T) -> Double): BinModel<Double, C> = BinModel(
...
fun <T, C : Comparable<C>> BinModel<List<T>, C>.varianceByDouble(selector: (T) -> Double): BinModel<Double, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).variance()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.sumOfSquaresByDouble(selector: (T) -> Double): BinModel<Double, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).sumOfSquares()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.normalizeByDouble(selector: (T) -> Double): BinModel<DoubleArray, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).normalize()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.descriptiveStatisticsByDouble(selector: (T) -> Double): BinModel<Descriptives, C> = BinModel(
...
fun <K> Map<K, List<Double>>.intRange(): Map<K, ClosedFloatingPointRange<Double>> = entries.associate { it.key to it.value.doubleRange() }
...
fun <K> Map<K, List<Double>>.descriptiveStatistics(): Map<K, Descriptives> = entries.associate { it.key to it.value.descriptiveStatistics }
...
fun <K, V> Map<K, List<V>>.doubleRangeBy(selector: (V) -> Double): Map<K, ClosedFloatingPointRange<Double>> = entries.associate {
...
fun <K, V> Map<K, List<V>>.descriptiveStatisticsByDouble(selector: (V) -> Double): Map<K, Descriptives> = entries.associate {
