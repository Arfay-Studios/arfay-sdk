...
val FloatArray.descriptiveStatistics
   get(): Descriptives = DescriptiveStatistics().apply { forEach { addValue(it.toDouble()) } }.let(::ApacheDescriptives)
...
fun <T, C : Comparable<C>> BinModel<List<T>, C>.sumByFloat(selector: (T) -> Float): BinModel<Float, C> = BinModel(bins.map {
   Bin(
...
fun <T, C : Comparable<C>> BinModel<List<T>, C>.floatRangeBy(selector: (T) -> Float): BinModel<ClosedFloatingPointRange<Float>, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).floatRange()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.geometricMeanByFloat(selector: (T) -> Float): BinModel<Double, C> = BinModel(
...
fun <T, C : Comparable<C>> BinModel<List<T>, C>.sumOfSquaresByFloat(selector: (T) -> Float): BinModel<Double, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).sumOfSquares()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.normalizeByFloat(selector: (T) -> Float): BinModel<DoubleArray, C> = BinModel(
   bins.map { Bin(it.range, it.value.map(selector).normalize()) })

fun <T, C : Comparable<C>> BinModel<List<T>, C>.descriptiveStatisticsByFloat(selector: (T) -> Float): BinModel<Descriptives, C> = BinModel(
...
fun <K> Map<K, List<Float>>.floatRange(): Map<K, ClosedFloatingPointRange<Float>> = entries.associate { it.key to it.value.floatRange() }
...
fun <K> Map<K, List<Float>>.descriptiveStatistics(): Map<K, Descriptives> = entries.associate { it.key to it.value.descriptiveStatistics }
...
fun <K, V> Map<K, List<V>>.floatRangeBy(selector: (V) -> Float): Map<K, ClosedFloatingPointRange<Float>> = entries.associate {
...
fun <K, V> Map<K, List<V>>.descriptiveStatisticsByFloat(selector: (V) -> Float): Map<K, Descriptives> = entries.associate {
