...
interface Argumentable : Iterable<String> {
	...
	/**
	 * The name of the command that's holds all arguments.
	 */
	val command: String
	...
	fun string(
		index: Int,
		message: String = "§cArgumento de texto não reconhecido no index $index"
	): String = optionalString(index) ?: fail(message)
...
/**
 * Returns the size of the arguments of this argumentable.
 */
...
/**
 * Gets a optional char argument at the specified index
 * or null if the argument not exists.
 */
fun Argumentable.optionalChar(index: Int): Char? = optionalString(index)?.firstOrNull()

/**
 * Gets a char argument at the specified index or throws
 * a [InstructorError] if the argument not exists.
 */
fun Argumentable.char(
	index: Int,
	message: String = "§cCaractere não reconhecido no index $index"
): Char = optionalChar(index) ?: fail(message)

/**
 * Gets a optional char array argument at the specified index
 * or null if the argument not exists.
 */
fun Argumentable.optionalCharArray(index: Int): CharArray? = optionalString(index)?.toCharArray()

/**
 * Gets a char array argument at the specified index or throws
 * a [InstructorError] if the argument not exists.
 */
fun Argumentable.charArray(
	index: Int,
	message: String = "§cCaracteres não reconhecidos no index $index"
): CharArray = optionalCharArray(index) ?: fail(message)

/**
 * Gets a optional boolean argument at the specified index
 * or null if the argument not exists.
 */
fun Argumentable.optionalBoolean(index: Int): Boolean? = optionalString(index)?.equals("true", true)
...
fun Argumentable.optionalOfflinePlayer(index: Int): OfflinePlayer? {
	...
	return if (!player.hasPlayedBefore()) null else player
}
 ...
 * or null if the argument not exists.
 */
fun Argumentable.optionalMaterial(index: Int): Materials? {
	val string = optionalString(index) ?: return null
	return Materials.parse(string)
}
...
): Materials = optionalMaterial(index) ?: fail(message)
...
fun Argumentable.optionalMaterialData(index: Int): MaterialData? {
	...
	val material = Material.getMaterial(split[0]) ?: Material.getMaterial(split[0].toInt()) ?: return null
 ...
 */
fun Argumentable.limit(size: Int): List<String> = take(size)
